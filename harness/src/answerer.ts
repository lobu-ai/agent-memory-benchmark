...
export function buildContextText(
  ...
  if (contextPrefix?.trim()) {
    sections.push(
      `Supplemental context (non-citable background):\n${contextPrefix.trim()}`
    );
  }
...
function findProperNouns(value: string): string[] {
  return (
    value.match(/\b[A-Z][A-Za-z0-9]+(?:\s+[A-Z][A-Za-z0-9]+){0,3}\b/g) ?? []
  );
}

function findMentionedPhrase(
  question: string,
  phrases: string[]
): string | null {
  ...
  for (const item of items) {
    ...
    return {
      answer: normalizeAnswer(
        transform ? transform(match) : (match[1] ?? "unknown")
      ),
      citedIds: [item.id],
    };
  }
...
function pickSentenceForQuestion(
  question: string,
  items: RetrievedMemory[]
): AnswerResult | null {
  ...
  if (mentioned) {
    const matchingItem = items.find((item) =>
      normalize(item.text).includes(normalize(mentioned))
    );
...
function answerPreference(
  ...
  if (asksForPerson) {
    return pickByRegex(
      items,
      /^([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)*)\s+prefers\s+/i,
      (m) => m[1]!.trim()
    );
  }

  return pickByRegex(items, /prefers\s+([A-Za-z][A-Za-z\s-]+)\.?$/i, (m) =>
    m[1]!.trim()
  );
}

function answerEmployment(
  question: string,
  items: RetrievedMemory[]
): AnswerResult | null {
  const q = normalize(question);
  const employmentFacts = items
    ...
    .filter(
      (
        value
      ): value is { item: RetrievedMemory; person: string; company: string } =>
        value !== null
    );
  ...
  if (q.startsWith("who")) {
    const fact = mentionedCompany
      ? employmentFacts.find(
          (entry) => normalize(entry.company) === normalize(mentionedCompany)
        )
      : employmentFacts[0];
  ...
  return firstFact
    ? { answer: firstFact.company, citedIds: [firstFact.item.id] }
    : null;
}
...
function answerBudget(
  ...
  const q = normalize(question);
  const wantsOriginal =
    q.includes("original") || q.includes("initial") || q.includes("before");
  const wantsLatest =
    q.includes("latest") ||
    q.includes("current") ||
    q.includes("after") ||
    q.includes("revision");
  const mentionedEntity = findMentionedPhrase(
    question,
    findProperNouns(question)
  );

  const entityScopedItems = mentionedEntity
    ? items.filter((item) =>
        normalize(item.text).includes(normalize(mentionedEntity))
      )
    : items;
  ...
  const preferredItems = entityScopedItems.filter((item) => {
    ...
    if (wantsOriginal)
      return !text.includes("revised") && !text.includes("later");
  ...
  return pickByRegex(candidateItems, /(\$?\d[\d,]*)/i, (m) =>
    m[1]!.replace(/,/g, "")
  );
}
...
function answerBudgetChange(items: RetrievedMemory[]): AnswerResult | null {
  return (
    ...
    pickByRegex(items, /([^.]*(?:delayed|changed)[^.]+)\.?$/i, (m) =>
      m[1]!.trim()
    )
  );
}
...
function answerDecision(
  ...
  const q = normalize(question);
  if (
    q.startsWith("which project") ||
    q.startsWith("what project") ||
    q.startsWith("who")
  ) {
    return pickByRegex(
      items,
      /(Project\s+[A-Z][A-Za-z0-9]+).*decided\s+to\s+/i,
      (m) => m[1]!.trim()
    );
  }
...
function answerLead(
  ...
  if (mentionedPerson) {
    return pickByRegex(
      items,
      new RegExp(
        `^${escapeRegex(mentionedPerson)}\\s+is\\s+(?:now\\s+)?the\\s+([^.]*)\\.?$`,
        "i"
      ),
      (m) => m[1]!.trim()
    );
  ...
  if (asksForPerson) {
    const mentionedLocation = findMentionedPhrase(
      ...
      items
        .map(
          (item) =>
            item.text.match(/based\s+in\s+([A-Z][A-Za-z0-9\s.-]+)\.?$/i)?.[1]
        )
...
function answerManages(items: RetrievedMemory[]): AnswerResult | null {
  return pickByRegex(
    items,
    /^([A-Z][A-Za-z]+(?:\s+[A-Z][A-Za-z]+)*)\s+manages\s+/i,
    (m) => m[1]!.trim()
  );
}

function answerOwnership(
  question: string,
  items: RetrievedMemory[]
): AnswerResult | null {
  const q = normalize(question);
  ...
  const partRelations = items
    ...
    .filter(
      (
        value
      ): value is { item: RetrievedMemory; feature: string; product: string } =>
        value !== null
    );
  ...
  const ownerRelations = items
    ...
    .filter(
      (
        value
      ): value is { item: RetrievedMemory; team: string; product: string } =>
        value !== null
    );
  ...
  async answer(
    ...
    if (q.includes("prefer") || q.includes("drink") || q.includes("beverage")) {
      return (
        ...
        pickSentenceForQuestion(question, items) ?? {
          answer: "unknown",
          citedIds: [],
        }
      );
    ...
    if (
      (q.includes("team") && q.includes("own")) ||
      q.includes("owned by which team")
    ) {
      return (
...
// Global cap on concurrent answerer requests across ALL systems running in
// parallel. The shared answerer (e.g. z.ai glm-5.1) rate-limits aggressively;
// without this, N systems x parallel questions burst past the provider's RPM
// and every system dies with 429 (observed with a 5-way run). Tunable via
// ANSWERER_MAX_CONCURRENCY (default 3).
const ANSWERER_MAX_CONCURRENCY = Math.max(
  1,
  Number.parseInt(process.env.ANSWERER_MAX_CONCURRENCY ?? "3", 10) || 3
);
let answererActive = 0;
const answererWaiters: Array<() => void> = [];
async function acquireAnswererSlot(): Promise<() => void> {
  if (answererActive >= ANSWERER_MAX_CONCURRENCY) {
    await new Promise<void>((resolve) => answererWaiters.push(resolve));
  }
  answererActive += 1;
  let released = false;
  return () => {
    ...
    released = true;
    answererActive -= 1;
    const next = answererWaiters.shift();
    if (next) next();
  };
}
...
class OpenAiCompatibleAnswerer implements BenchmarkAnswerer {
  ...
  constructor(config: OpenAiCompatibleAnswererConfig) {
    ...
    this.missingKeyError = () =>
      new Error(`Missing answerer API key env '${envVar}'`);
  }
  ...
  private async requestCompletion(args: {
    ...
    const maxAttempts = 6;
    const release = await acquireAnswererSlot();
  ...
  private backoffMs(attempt: number, retryAfter: string | null): number {
    const headerMs = retryAfter ? Number.parseFloat(retryAfter) * 1000 : NaN;
    const base = Number.isFinite(headerMs)
      ? headerMs
      : 750 * 2 ** (attempt - 1);
    // +/-30% jitter so parallel systems don't retry in lockstep.
...
export function createAnswerer(
  config: AnswererConfig | undefined
): BenchmarkAnswerer | null {
