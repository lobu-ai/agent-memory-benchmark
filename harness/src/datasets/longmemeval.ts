...
export interface LongMemEvalRecord {
  ...
  haystack_sessions: Array<
    Array<{ role: string; content: string; has_answer?: boolean }>
  >;
...
function renderSessionContent(
  ...
  const renderedTurns = turns
    .map((turn, index) => {
      ...
      // Do NOT inject a gold-evidence marker into the content. `has_answer` is a
      // benchmark annotation marking which turn holds the answer; putting it in
      // the text the reader model sees leaks the answer location to every system.
      return `Turn ${index + 1} (${role}): ${normalizeWhitespace(turn.content)}`;
    })
    .join("\n\n");
...
function toScenario(
  ...
  const zipped = record.haystack_session_ids.map((sessionId, sessionIndex) => ({
    sessionId,
    sessionDate:
      record.haystack_dates[sessionIndex] ?? `unknown-${sessionIndex + 1}`,
    sessionTurns: record.haystack_sessions[sessionIndex] ?? [],
  }));

  const steps: BenchmarkStep[] = zipped.map(
    ({ sessionId, sessionDate, sessionTurns }) => ({
...
export function convertLongMemEvalToBenchmarkSuite(
  ...
  if (sliced.length === 0) {
    throw new Error(
      "No LongMemEval records matched the requested limit/offset"
    );
  }
  ...
  return {
    id:
      options.suiteId ??
      `longmemeval-${options.variant}-${options.limit ?? sliced.length}`,
    ...
    entityTypes: [
      {
        ...
        eventKinds: {
          conversation_session: {
            description: "Timestamped conversation history session",
          },
        },
      },
    ],
    scenarios: sliced.map((record, index) =>
      toScenario(record, offset + index)
    ),
  };
}
