...
import type { BenchmarkRunConfig, CommandSystemConfig } from "./types";
...
function resolveEnvValue(value: string): string {
  ...
  if (optional) {
    const v = process.env[optional[1]!];
    return v != null && v !== "" ? v : optional[2]!;
  }
  ...
  if (!resolved) {
    throw new Error(
      `Missing environment variable '${envName}' referenced from benchmark config`
    );
  }
  return resolved;
}
...
export function loadBenchmarkConfig(path: string): BenchmarkRunConfig {
  ...
  const systems = config.systems
    ...
    .map((system) => {
      ...
      throw new Error(
        `Unsupported benchmark system type '${(system as { type?: string }).type ?? "unknown"}'. Every system must be an external 'command' adapter.`
      );
    });
  ...
  return {
    ...
    outputDir: resolve(process.cwd(), config.outputDir ?? "results"),
