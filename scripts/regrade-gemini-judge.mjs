#!/usr/bin/env node
/* Gemini-judge regrade via the Gemini CLI (`gemini -p`) — runs on a Google
 * account login, no API key required (the earlier API-keyed gemini judging
 * hit its monthly spend cap; the CLI quota is separate).
 *
 * Grading rubric is byte-identical per item to scripts/regrade-zai-judge.mjs
 * and scripts/regrade-claude-judge.mjs; items are batched per CLI call.
 * Judges the SAME way on every file passed, so cross-file deltas are fair.
 * Usage:
 *   node scripts/regrade-gemini-judge.mjs results/A.json results/B.json
 *
...
async function judgeBatch(items) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    try {
      const verdicts = parseBatchVerdicts(await geminiCall(prompt), items.length);
...
for (const file of process.argv.slice(2)) {
  ...
  for (const sys of data.systems) {
    ...
    // Empty answers are wrong by definition — no judge call can save them.
    ...
    console.log(`  OVERALL gemini-judge: ${pct(allNew)?.toFixed(1)}%`);
